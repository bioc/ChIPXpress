...
\\[3ex]
\noindent
(2) \texttt{ChIP}, Vector of TF bound genes predicted from ChIPx data.
\begin{small}
...
software of their preference and only need to report the genes (in any order) predicted to be bound by the TF-of-interest.
\end{small}
\\[3ex]
\noindent
...
The output is a list of size two. The first item in the list is the Oct4 target gene rankings, where the names of the vector correspond to the Entrez GeneID of each gene and each individual value is the absolute correlation of the gene and Oct4 in the database. The second item reports the TF bound genes that were not found in the database (i.e. not measured by the microarray platform).
